...
(async () => {
    const totalReff = readlineSync.question('[!] Total Refferal : ')
    const codeReffKamu = fs.readFileSync('codereff.txt', 'UTF-8');
    const apikey = fs.readFileSync('apikey.txt', 'UTF-8');

    for (let index = 0; index < totalReff; index++) {
    ...
    start_position: while (true) {
        ...
        if (!otp) {
            if (waktu()-time>30) {
                ...
                console.log('    Failure Get Code 30 Second Auto Delete Order ID')
                break;
            } else {
